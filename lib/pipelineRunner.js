...
export async function runOptimizationPipeline(
  ...
  // Save manual input to history if it's not part of a suite
  if (!suiteIdParam && input) {
    await inputManager.saveInput(input);
  }
  if (!suiteIdParam) {
  ...
  if (promptIdParam && promptIdParam !== "") {
    ...
    if (!promptData) {
      logger(
        `[Error] Prompt Cluster ${promptIdParam} not found. Falling back to demo.`,
      );
      ...
      promptData = await promptManager.addVersion(
        promptId,
        "Highly Structured Professional Output",
      );
    }
  } else {
    promptId = `demo-${runTimestamp}`;
